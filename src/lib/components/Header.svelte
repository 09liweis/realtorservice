...
<header class="bg-white border-b border-gray-100 fixed w-full top-0 z-50 shadow-sm">
	<nav class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
		<div class="flex justify-between items-center h-16">
			<!-- Logo -->
			<div class="flex items-center">
				<Link href="/" className="flex items-center space-x-2">
					<img src="/logo.png" alt="Realtor Service Logo" class="h-8" />
					<span class="text-xl font-bold text-gray-900 hidden sm:block">Realtor Service</span>
				</Link>
			</div>
				...
				{#each publicNavigation as item}
					<Link
						...
						className="text-gray-600 hover:text-primary px-3 py-2 text-sm font-medium transition-colors duration-200 {
							$page.url.pathname === item.href ? 'text-primary font-semibold' : ''
						}"
					>
						{item.name}
					</Link>
			...
			<div class="flex items-center space-x-4">
				{#if $user}
					...
					<div class="relative">
						...
						>
							<div class="w-8 h-8 bg-gradient-to-br from-blue-600 to-purple-600 rounded-full flex items-center justify-center text-white font-medium text-sm">
		...
		{#if mobileMenuOpen}
			<div class="md:hidden">
				<div class="px-2 pt-2 pb-3 space-y-1 sm:px-3 bg-white border-t">
					...
					{:else}
						<div
							class="border-t border-gray-200 mt-4 pt-4 space-y-2"
						>
							<Link
								href="/login"
