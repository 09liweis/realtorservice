<script lang="ts">
  ...
  import { upsertSocialMediaService } from '$lib/supabase';
  ...
  $: currentStatusInfo = SOCIAL_MEDIA_STATUS.find(s => s.value === socialMediaService.status) || SOCIAL_MEDIA_STATUS[0];

  // Get next possible statuses based on current status and user role
  $: nextStatuses = getNextStatuses(socialMediaService.status, $user?.isAdmin);
  ...
  function getNextStatuses(currentStatus: string, isAdmin: boolean) {
    ...
    switch (currentStatus) {
      ...
      case 'approved':
        return [
          { value: 'active', label: 'Start Service', color: 'green' }
        ];
      case 'active':
        return [
          { value: 'paused', label: 'Pause Service', color: 'orange' },
          { value: 'completed', label: 'Mark as Completed', color: 'green' }
        ];
      case 'paused':
        return [
          { value: 'active', label: 'Resume Service', color: 'green' },
          { value: 'completed', label: 'Mark as Completed', color: 'green' }
        ];
      default:
        return [];
  ...
  async function updateStatus(newStatus: string) {
    try {
      ...
      const updatedSocialMediaService = {
        ...socialMediaService,
        status: newStatus,
        updated_at: new Date().toISOString()
      };

      const { error: updateError } = await upsertSocialMediaService(updatedSocialMediaService);
      if (updateError) throw updateError;

      socialMediaService.status = newStatus;
      success = true;
      ...
      console.error('Error updating social media service status:', err);
    } finally {
      statusLoading = false;
    }
  }
  ...
  async function updatePrice() {
    try {
      ...
      const { error: updateError } = await upsertSocialMediaService(socialMediaService);
      if (updateError) throw updateError;
      ...
      console.error('Error updating social media service price:', err);
    } finally {
      statusLoading = false;
    }
  }
...
<div class="bg-white rounded-xl shadow-sm border border-gray-200 sticky top-6">
  ...
  <div class="p-6 space-y-4">
      ...
      <div class="flex items-center space-x-3">
        <div class="w-8 h-8 bg-{currentStatusInfo.color}-100 rounded-lg flex items-center justify-center">
          <span class="text-lg">
            ...
            {:else if currentStatusInfo.value === 'active'}🚀
            {:else if currentStatusInfo.value === 'paused'}⏸️
    ...
    <!-- Admin Price Update -->
    {#if $user?.isAdmin && (socialMediaService.status === 'pending' || socialMediaService.status === 'quoted')}
      <div class="space-y-3">
        <Input
    ...
    {:else}
      <div class="text-center py-4">
        <div class="w-12 h-12 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-3">
          <span class="text-2xl">
            {#if socialMediaService.status === 'completed'}🎉
            {:else if socialMediaService.status === 'cancelled'}❌
            {:else if socialMediaService.status === 'active'}🚀
            {:else}⏳{/if}
          </span>
        </div>
        ...
        <div class="text-xs text-gray-500 mt-1">
          {#if socialMediaService.status === 'completed'}
            Your social media service is complete
          {:else if socialMediaService.status === 'cancelled'}
            This service has been cancelled
          {:else if socialMediaService.status === 'active'}
            Your social media service is currently active
          {:else}
    ...
    <div class="mt-6 p-4 bg-gray-50 rounded-lg">
      <h3 class="text-sm font-medium text-gray-700 mb-3">Service Information</h3>
      <div class="space-y-2 text-xs text-gray-600">
        <div class="flex justify-between">
          <span>Service ID:</span>
          <span class="font-mono">{socialMediaService.id?.slice(-8) || 'N/A'}</span>
        </div>
        <div class="flex justify-between">
          <span>Created:</span>
          <span>{socialMediaService.created_at ? new Date(socialMediaService.created_at).toLocaleDateString() : 'N/A'}</span>
        </div>
        <div class="flex justify-between">
          <span>Last Updated:</span>
          <span>{socialMediaService.updated_at ? new Date(socialMediaService.updated_at).toLocaleDateString() : 'N/A'}</span>
        </div>
      </div>
    </div>
  </div>
